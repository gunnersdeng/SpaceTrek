...
- (void)preLoadSoundFiles
{
    ...
    [[SimpleAudioEngine sharedEngine] preloadEffect:@"level3Background.mp3"];
    ...
    [[SimpleAudioEngine sharedEngine] preloadBackgroundMusic:@"level3Background.mp3"];
    [[SimpleAudioEngine sharedEngine] playBackgroundMusic:@"level3Background.mp3"];

    //firstBackgroundMusic = [[SimpleAudioEngine sharedEngine]playEffect:@"level3Background.mp3"];
    ...
    if(isCollectCircle_3)
    {
        for(b2Body *b = world->GetBodyList(); b; b=b->GetNext()) {
            if (b->GetUserData() != NULL) {
                ...
                if(treasureData.tag!=SPACESTATION_TAG && (sqrt(sqr(b->GetPosition().x-position.x)+sqr(b->GetPosition().y-position.y))<3)&&isCollect_3)
                {
                    CCLOG(@"here 0");
...
//    [treasure setType:gameObjectTreasure1];
    ...
    blackholeShapeDef.filter.categoryBits = 0x40;
